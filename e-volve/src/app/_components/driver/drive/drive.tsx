...
import { Card, Spin, Button, Tooltip, Badge } from 'antd';
import { Loader } from '@googlemaps/js-api-loader';
import { Navigation, Map, Locate, LocateFixed, Info, Clock } from 'lucide-react';
...
const NavigationComponent: React.FC<NavigationComponentProps> = ({ destination }) => {
  ...
  useEffect(() => {
    ...
    loader.importLibrary('maps')
      ...
      .catch(err => {
        ...
        setError('Failed to load maps. Please check your connection and reload.');
        setLoading(false);
      });
  }, [destination, mapRef.current]);

  // Track driver's live position
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        setError('Location access is required for navigation. Please enable location services.');
      },
  ...
  useEffect(() => {
    ...
    const listener = window.google.maps.event.addListener(map, 'dragstart', () => {
      setFollowDriver(false);
    });
  ...
  if (error) {
    return (
      <Card
        title={<div className="flex items-center"><Navigation className="mr-2" />Navigation Error</div>}
        bordered={false}
  ...
  return (
    <Card
          ...
          {eta && distance && (
            <div className="flex items-center">
              <Tooltip title="Estimated arrival time">
                <Badge count={eta} style={{ backgroundColor: '#52c41a' }}>
      ...
      bordered={false}
      style={{ width: '100%', height: '100vh' }}
      bodyStyle={{ padding: 0, height: 'calc(100% - 58px)', position: 'relative' }}
    >
          ...
          >
            <Tooltip title="Center on your location">
              <Button 
                ...
                style={{ backgroundColor: followDriver ? '#1890ff' : '#fff', color: followDriver ? '#fff' : '#1890ff' }}
              />
            </Tooltip>
